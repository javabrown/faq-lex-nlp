...
def help_icw_user(lex):
    try:
        faq = FaqData("template/qa.csv")
        ans = faq.findAnswer(lex.getUserInputText())

        if ans.lower() == 'no_answer' or ans.lower() == '' or ans  == 'NO_ANSWER':
            return not_understood(lex)

        return lex.fulfill( faq.findAnswer(lex.getUserInputText()) )
...
def icw_dispatch(lex):
    # return help_icw_user(lex)
    if lex.intent == "iCWFAQIntents":
        return help_icw_user(lex)
    else:
        return not_understood(lex)
...
def lambda_handler(event, context=None):
    ...
    return icw_dispatch(lex)


# faq = c.faq_data.FaqData("template/qa.csv")
# # faq = FaqData("metainfo/qa.csv")
# print("Hello from lambda - " + faq.findAnswer("how to get a loan"))
